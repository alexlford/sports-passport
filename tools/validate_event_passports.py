...
if not sitemap.is_file():
    ...
    try:
        ...
        for item in ranked:
            event_id = item.get("event_id")
            expected = f"{ORIGIN}/event.html?id={event_id}"
            if expected not in urls:
                errors.append(f"sitemap missing ranked exact-event URL: {expected}")
...
print(
    f"OK: exact event passport template, {len(linked_pages)} deep-linking views, "
